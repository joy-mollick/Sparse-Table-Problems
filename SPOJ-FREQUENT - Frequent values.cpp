...
/// Very Classical problem on RMQ 
/// It is said that it is sorted so , same numbers will be consecutively side by side so from left side count the number of occurrences of that numbers 
/// Then store them another array and make sparse table over that  array 
/// There is one problem when starting will be from an mid interval of some same number's interval , so cover that problem with an extra loop 
/// Then it is just a RMQ problem 
